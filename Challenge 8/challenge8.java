...
static class Pair<U, V>
{
 ...
 public static List<String> convertStringToList(String str) {
    ...
    public static String findCorrectPath(String instructions, List<Integer> target, List<List<Integer>> obstacles) {
        ...
        convertStringToList(instructions).forEach(instr -> moves.add(Movement.getMove(instr)));

        System.err.println("Obstacles:\n" + obstacles);
        List<Pair<Integer,Integer>> obs = new ArrayList<>();
        obstacles.forEach(o -> {
            obs.add(Pair.of(o.get(0), o.get(1)));
        });
        ...
        int index = 0;
        Movement instruction = Movement.BACK;
        ...
        String result = "Replace instruction " + index + " with " + instruction.toString();
        System.err.println(result);
        return result;
    }

    public static boolean tryToMoveToObstacle(List<Pair<Integer, Integer>> obstacles, PositionAndOrientation init, Movement move) {
        if (!move.isStep()) {
            return false;
        } else {
            PositionAndOrientation endResult = move(init, move);
            if (obstacles.contains(endResult.getCoords())) {
                return true;
            } else {
                return false;
            }
    ...
    FORWARD("F"), BACK("B"), TURN_LEFT("L"), TURN_RIGHT("R");
    ...
    public static Movement getMove(String text) {
        if (text.equals("F")) return FORWARD;
        else if (text.equals("B")) return BACK;
        else if (text.equals("L")) return TURN_LEFT;
        else if (text.equals("R")) return TURN_RIGHT;
        return null;
    }
    ...
    public String toString() {
        ...
        else if (text.equals("L")) return "TURN LEFT";
        else /*if (text.equals("R"))*/ return "TURN RIGHT";
    }

    public boolean isStep() {
        return (this.text == "F" || this.text == "B");
    }

    public Movement getOpposite() {
        if (text.equals("F")) return BACK;
        else if (text.equals("B")) return FORWARD;
        else if (text.equals("L")) return TURN_RIGHT;
        else if (text.equals("R")) return TURN_LEFT;
        return null;
    }
}
